...
function []= compConnFromSubjIDEYESOPEN(sourceFilePath,subjectID,destinationPath,locPath,locPathFrwrdMdel,chckModelConsistancy,drug,varargin)
%
%
...
%importing 
% choosing the selecting Eyes open resting state
EEG = pop_rmdat( EEG, {'S  1'},[-0.001 1.999] ,0);EEG.setname=int2str(subjectID);EEG = eeg_checkset( EEG );pop_eegplot( EEG, 1, 1, 1);
...
if drug=='off'
    save([destinationPath,'EEGObj_offDrug_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
elseif drug=='onn'
    save([destinationPath,'EEGObj_OnDrug_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
elseif drug=='Cnt'
    save([destinationPath,'EEGObj_Cntrl_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
end

% if drug=='off'
%     load([destinationPath,'EEGObj_offDrug_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
% elseif drug=='onn'
%     load([destinationPath,'EEGObj_OnDrug_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
% elseif drug=='Cnt'
%     load([destinationPath,'EEGObj_Cntrl_EYESOPEN',int2str(subjectID),'.mat'],'EEG')
% end
...
EEG= pop_est_mvarConnectivity(EEG,'nogui','connmethods',{'ffDTF','dDTF','RPDC'}); %{'DTF','dDTF','ffDTF','RPDC','Coh','pCoh'}
...
if drug=='off'
    save([destinationPath,'CompffDTFMatrx_EYESOPEN_',int2str(subjectID),'OFFDRUG','.mat'],'ffDTF')
    save([destinationPath,'CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'OFFDRUG','.mat'],'dDTF')
    save([destinationPath,'CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'OFFDRUG','.mat'],'RPDC')
elseif drug=='onn'
    save([destinationPath,'CompffDTFMatrx_EYESOPEN_',int2str(subjectID),'ONDRUG','.mat'],'ffDTF')
    save([destinationPath,'CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'ONDRUG','.mat'],'dDTF')
    save([destinationPath,'CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'ONDRUG','.mat'],'RPDC')
elseif drug=='Cnt'
    save([destinationPath,'CompffDTFMatrx_EYESOPEN_',int2str(subjectID),'.mat'],'ffDTF')
    save([destinationPath,'CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'.mat'],'dDTF')
    save([destinationPath,'CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'.mat'],'RPDC')
end
