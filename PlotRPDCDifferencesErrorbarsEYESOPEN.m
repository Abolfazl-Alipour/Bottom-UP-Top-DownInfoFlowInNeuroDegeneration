...
CntrlList=[894	908	8010		903	8060	893	909	911	895	913		900	899	914	910	890	912	905	904	892	902	901		897		907];
%(subject 906 (15.50 secs) and subject  891 (9.5 secs) were removed from
%the list) 
CntrlRPDCMat=nan(length(CntrlList),upperFrequencyBound);
for i=1:length(CntrlList)
    subjectID=CntrlList(i);
    RPDCFileName=strcat('CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'.mat');
...
for i=1:length(PDList)
    subjectID=PDList(i);
    RPDCFileName=strcat('CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'OFFDRUG','.mat');
    load(strcat(destinationPath,RPDCFileName))
...
%%%%%%
PDListOnDrugCntrl=[894	908	8010	903	8060	893	909	911	895	913		900	899	914	910	890	912	905	904	892	902	901		897		907];
%(subject 906 (15.50 secs) and subject  891 (9.5 secs) were removed from
%the list)
...
PDListONDrug=[801	802	803	804	805	806	807	808	809	810	811	813	815	816	817	818	819	820	821	822	823		825	827		829];%826	;
% 828 (12.34 secs) was too short and therefeore it was removed, 824 as well
...
for i=1:length(PDListONDrug)
    ...
    RPDCFileName=strcat('CompRPDCMatrx_EYESOPEN_',int2str(subjectID),'ONDRUG','.mat');
    load(strcat(destinationPath,RPDCFileName))
...
title('Infromation flow in mid beta band')

ylabel('Average normalized rPDC (17-20 Hz)') 
ax=gca;
...
x = 1:2;
Cntrl=mean(normalizedPDONCntrlRPDCMat(:,lowerBound:higherBound),2);
PD=mean(normalizedPDONCntrlRPDCMat(:,lowerBound:higherBound),2);
data=[mean(Cntrl),mean(PD)];
...
ax = gca;
ax.FontSize = 28;
ax.YLim=[0.0006 0.0016];
% set(get(get(h(1),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% set(get(get(h(2),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
legend({'PD Patients Off Drug','{\it R^2}=.1610, p=0.04'},'Location','northwest')




PDListONDrugLED=[697	1275	600	600	250	520	550	1150	400	300	200	600	500	400	640	600	600	700	60	1650	1000	600	1796	338];
...
% set(get(get(h(1),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% set(get(get(h(2),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
legend({'PD Patients On Drug','{\it R^2}=.0001, p=0.95'},'Location','northwest')
