...
for i=1:length(PDOFFList)
    ...
    RPDCFileName=strcat('CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'OFFDRUG','.mat');
...
for i=1:length(CntrlList)
    ...
    RPDCFileName=strcat('CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'.mat');
    load(strcat(destinationPath,RPDCFileName))
...
for i=1:length(PDListONDrug)
    ...
    RPDCFileName=strcat('CompdDTFMatrx_EYESOPEN_',int2str(subjectID),'ONDRUG','.mat');
    load(strcat(destinationPath,RPDCFileName))
    tmpTime=mean(dDTF(:,:,1:58,:),4);
    infOutflow=nan(1,size(tmpTime,3));
